#ifndef __FIXED_ADDR_ALLOCATOR_HPP__
#define __FIXED_ADDR_ALLOCATOR_HPP__
...
/*
TODO: Seriously consider abstracting pool/arena state,
and think about how tightly the allocator is bound to its pool.
Up until now I think it's correct that the allocator is fully coupled
to a specific pool, but we'll have to think about how this will later work with
    1. copying and moving of rdma-transferrable containers, and
    2. dynamic unbounded pool sizes (which will build on the above).
Also, perhaps it would be cleaner to have pool-specific logic moved out of
the allocator?
*/
...
public: // Boilerplate.
    ...
    using is_always_equal                        = std::is_empty<FixedAddrAllocator>;

    template <class U> friend class FixedAddrAllocator;
    template <class U> struct rebind {typedef FixedAddrAllocator<U> other;};

private:
    // The allocator is NOT responsible for this memory pool object.
    // Multiple allocators may be using this memory pool.
...
public:
    ...
    // Constructor.
    FixedAddrAllocator(MemoryPool* mempool) noexcept : mempool(mempool) {
        std::cerr << "Creating FixedAddrAllocator using "
                  << mempool << std::endl;
    }
    ...
    FixedAddrAllocator() noexcept : mempool(NULL) {}

    template <class U>
    FixedAddrAllocator(FixedAddrAllocator<U> const& a) noexcept :
        mempool(a.mempool) {}
    ...
    FixedAddrAllocator select_on_container_copy_construction() const {
        return *this;
    }
};
...
bool operator==(
    FixedAddrAllocator<T> const& alloc_a, FixedAddrAllocator<U> const& alloc_b
) noexcept {
    // False for safety.
...
template <class T, class U>
bool operator!=(FixedAddrAllocator<T> const& x, FixedAddrAllocator<U> const& y) noexcept {
    return !(x == y);
}

#endif // __FIXED_ADDR_ALLOCATOR_HPP__